...
**✅ Pi-L3-Welle GRÜN (2026-07-17, `hardware-validator`, Pi 3B aarch64, Deploy von main via rsync + Swap-Docker-Build):**
- **B3** 3 Trigger Δ=1.059/1.058/1.020 s (<2 s), Long-Poll kehrt vor 25-s-Idle zurück — PASS
- **B6** 6 Skip-Zyklen, 0 Panel-Writes, `eink_last_sent.png` mtime unverändert — PASS
- **B5-live** Temp „Draußen: 28.3°C" + Presence „Kilian ist: Zuhause" (echte HA-Werte, von Kilian bestätigt); alarm-Modus nicht live konfiguriert (code-covered) — PASS
- **B2** Panel vs `/preview` 0.000000 % Pixel-Diff, Palette exakt 6 Spectra, Radius live = nominal R — PASS (**L4 Panel-Foto offen: keine Kamera**)
- **B1** Wetter+Forecast render korrekt — PASS
- **Token-Sicherheit** 0 Log-Leaks, hass.json 0600, Token nur dort; App-Passwort nur zur Session genutzt, nie geloggt — PASS
- **Ressourcen** Client 37.7 MB / Server 20.2 MB (VmHWM-Peak 74.9 MB transient, notiert) — PASS
- **ARM/go1.25** Image baut+läuft nativ auf dem Pi (Deps-L3-ARM auch off-Pi validiert: arm64+armv7 Cross-Compile grün)
...
**OFFENE KILIAN-ASKS:** (b) **Release-Freigabe** wenn bereit — **einziger offener Punkt.** [(a) B5-Token via Web-UI + (c) Pi-Neustart: erledigt.]
...
**Session #2 Fortschritt (2026-07-16, forts.):**
- **B3 gemerged** `main` (`c346d16` Server + `95f6ddb` Client, Squash, Integrations-L1 grün). B3-L3 (<2s Button→Panel, AC22) **gebündelt** mit weiteren L3-Checks (ein Deploy → minimiert Panel-Writes + Disruption des Live-Pi).
- **Deps:** Dependabot-Liste erhalten. Scope = **Go-Server**, NICHT Pillow/requests: `golang.org/x/crypto` (8 kritisch — alle in **ungenutztem** `x/crypto/ssh`; Server nutzt nur `bcrypt` → nicht ausnutzbar, Bump schließt Alerts) + `golang.org/x/image` (2 — TIFF, im Renderer). Bump `x/crypto`+`x/image`; Go 1.24 bleibt (Toolchain-Bump separat). x/image im Render-Pfad → L2-Golden-Check + L3-Render pending.
- **B5 Home Assistant:** Kilian gibt frei + liefert **read-only Token** (`10.33.0.5:8123`, in `secrets.local.md` gitignored, 10y). `security-reviewer` prüft Token-Speicher + SSRF-Policy (configured-URL-Allowlist für interne IP) — **Pflicht-APPROVE vor Bau**. B5 wird auf die **B4-Unified-Pipeline** gebaut (nicht auf den Doppel-Pfad).
- **B4:** Spec `specs/B4-unify.md` fertig (B4a Server-Dispatcher+`POST /api/widget_content`+Content-Gleichheitstest → B4b Canvas-Passthrough → B4c Löschen dead-E-Pfad). Erfreulich: `fill*Content` sind bereits reine Content-Builder (kein Extrahieren nötig), Golden-Re-Baseline = No-Op.

**Session #2 — Merges & B4/B5 (2026-07-16, forts.):**
- **B4a DONE** — gemerged `main`@`8e4c418` (Squash): `WidgetTextContent`-Dispatcher + `POST /api/widget_content`, `drawElement` ohne Fork, **Golden byte-identisch** (2 unabhängige Reviews MERGE-READY). **B4b** (Canvas-Passthrough, Client-Formatter + Fake-Daten raus, `textContent`/Fabric statt innerHTML) läuft.
- **`.gitignore`-Härtung DONE** (`4545e8a`): `data/auth.json` / `data/hass.json` / `secrets.local.md` ignoriert.
- **B5 Security-Gate: APPROVED TO BUILD** (X/Y/Z: Token nie Prop/URL/Log/Preview/Export + SSRF-Allowlist, je mit Tests). Spec reconciled: **`hassMode`-Prop** (statt `layout`), **deutsche Strings via `locale.go`** (AC-HA7 single-source, grep-gated), L5-Härtungen. Dekomposition B5a→B5b→B5c. Offen an Kilian (nicht blockierend): exakte Entity-IDs + Alarm-Backend (`alarm_control_panel` vs `alarmo`) — L1/L2 mocken. **L5-Code-Security-Re-Review vor B5-Merge Pflicht.**
- **Deps: Entscheidung Kilian = „vor Release, eigene Runde".** Go 1.24→1.25 + `x/image`@v0.43.0 + `x/crypto`@v0.52.0 als isolierte Infra-Runde vor Release-Tag (CI + Pi-Cross-Build). Von 17 Alerts nur 1 erreichbar (x/image **SFNT-OOM** im Font-Render-Pfad; x/crypto-ssh ungenutzt), nicht akut.
- **Pi-Sammel-L3 (pending):** B3 <2s Button→Panel (AC22) + B6-Skip-Zählung + Deps-Render-Check — ein Deploy, minimiert Panel-Writes.

**Session #2 — B4/B5-Merges (2026-07-16, forts. 2):**
- **B4b DONE** (`498e883`, L1+L5+L2 grün) — Canvas==Panel live bewiesen: Kalender-Titel erscheint, keine Fake-Daten, Layout/Props wirken, clock/timer ticken. Damit sind die B4-Parität-Lücken strukturell behoben.
- **B5a DONE** (`3929ed9`, L1 + **Security-APPROVE**) — HA-Config (0600 atomar), gehärteter Safe-Fetch (Redirect aus, Dial-Pin, Allowlist, 2 MiB-Cap, Token nur Bearer), `/api/hass/config` gibt nie Token, Token nie geloggt (enforcing grep-gate). Kein echtes Token im Branch.
- **B2-Spec** fertig (`specs/B2-rounding-parity.md`): Kernbug = **Radius bei `high`-Qualität halbiert** (rx/sw nicht mit 2×-Supersample skaliert; nur bei `fast` korrekt) + Canvas-Scale-Drift + Transparent-Stroke-Bug. Fix 2-phasig (Radius-Parität / zentrierter Stroke-Ring). **Offene Kilian-Frage bei B2-Einplanung:** Stroke-Phase ändert sichtbar alle bestehenden gerahmten Shapes → zusammen oder separat freigeben.
- **B5b DONE** (L1+L5 grün, Security-Lens clean — Token nie im Content [AC-SEC3], deutsche Zustandstexte nur in `locale.go`, Content-Gleichheit Endpoint==Dispatcher==Panel, Golden byte-identisch, `SetHassService`-Wiring ohne Signaturänderung). **Danach:** B5c (Canvas-Props + HA-Settings-UI → dann L3-Live gegen HA `10.33.0.5:8123`), B4c (dead-E-Pfad löschen), B2 (Rundungs-Parität), B1 (Standortsuche), Pi-Sammel-L3, Go-1.25-Deps-Runde vor Release.
- **Hinweis Worktree-Isolation:** Worktrees basieren auf altem Session-HEAD (`3ecf4ce`) statt current main → Agenten müssen im Worktree zuerst `git merge --ff-only main` + `git checkout -B <branch>`; geteilten Checkout nicht anfassen.
- **B4 KOMPLETT + B5 code-complete (2026-07-16):** B4c (`808e217`, dead-E-Pfad gelöscht — Single-Content-Source, Doppel-Pfad eliminiert) + B5c (`377ca2b`, HA-Widget-UI, L2 inkl. XSS-Beweis `<img onerror>`→Literaltext) gemerged. **B5 offen nur noch L3-Live** gegen echte HA (Token + Entity-IDs von Kilian). **Verbleibend:** B2 (Kilian-Entscheid Stroke-Phase), B1 (Spec läuft), Pi-Sammel-L3 (B3<2s + B6-Skip + B5-live), Go-1.25-Deps-Runde vor Release. Offene Doku-Schuld: PROGRESS/specs erwähnen alten `/api/widgets/*`-Pfad historisch (Sweep vor Release).
