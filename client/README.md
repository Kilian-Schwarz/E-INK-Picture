...
| `EINK_CLIENT_TOKEN` | (empty) | Shared auth token, sent as `X-Client-Token` header on all server calls. Generated by `setup.sh`; must match `EINK_CLIENT_TOKEN` in the server `.env`. Empty = no header (only works while the server has no admin password set) |
